...
df = pd.read_table('DATA/Efield_iteration_5_Transmitted.dat', header=0)
eFieldT = df.values
...
df = pd.read_table('DATA/Efield_iteration_5_Reflected.dat', header=0)
eFieldR = df.values
...
df = pd.read_table('DATA/Spectrum_iteration_5_Transmitted.dat', header=0)
...
df = pd.read_table('DATA/Spectrum_iteration_5_Reflected.dat', header=0)
...
plt.clf()
plt.figure(figsize=(8, 6))
plt.semilogy(eSpectrumR[:halfR,0], eSpectrumR[:halfR,3], '-r', eSpectrumT[:halfT,0], eSpectrumT[:halfT,3], '-b')
plt.title(r'Reflected pulse spectrum')
plt.ticklabel_format(axis='x', style='sci', scilimits=(0,0))
plt.xlabel(r'$\omega$')

...
int dAdz(double z, const double y[], double f[], void *odep) {
	...
	if (odeObj->doPlasmaCalc == 3) { // MPI, Avalanche, & Recombination
		...
		for (int i = 0; i < num_t - 1; i++)
		{
			...
			current_change = ht * (pow(charge_e, 2) / mass_e) * odeObj->rho[i+1] * (fv1 + fv2) * 0.5 - ht * ve * current;
			fv1 = fv2;
		...
		for (int i = 0; i < num_t - 1; i++)
		{
			...
			// Second order method (Kolja)
			fv2 = real(odeObj->ee_p[i+1] + odeObj->ee_m[i+1]);
			current_change = ht * (pow(charge_e, 2) / mass_e) * odeObj->rho[i+1] * (fv1 + fv2) * 0.5 - ht * ve * real(odeObj->j_e[i]);
			fv1 = fv2;

...
//
{
  ...
  stddev = sqrt ( abs( sum_square - square_sum ) / ( double )( POPSIZE - 1 ) );
  ...
  //double x[NVARS+1];
	...
	for ( member = 0; member < POPSIZE; member++ )
	{
		for ( i = 0; i < NVARS; i++ )
		{
			...
			gsl_vector_set(u, i, population[member].gene[i]);
		} 
		population[member].fitness = -mapG(u, aRootParams);
	}
    gsl_vector_free(u);
	return;
}
...
void getBestGA(gsl_vector *v) {
    ...
    {
        gsl_vector_set(v, i, population[POPSIZE].gene[i]);
    }
}

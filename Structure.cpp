    ...
    if (numDimensionsMinusOne == 1) {
        ...
        for (int i = 1; i < numActiveOmega; i++)
        {
            if (i >= freqLowerCutoff && i <= freqUpperCutoff) {
                ...
                sp = (exp(-1.0i*(beta0[i] - beta1[i])*z)*(beta0[i] + beta1[i]) / (2.0*beta1[i]) * aPlus 
                        + exp(1.0i*(beta0[i] + beta1[i])*z)*(beta1[i] - beta0[i]) / (2.0*beta1[i]) * aMinus);

                sm = (exp(-1.0i*(beta0[i] + beta1[i])*z)*(beta1[i] - beta0[i]) / (2.0*beta1[i]) * aPlus 
                        + exp(1.0i*(beta0[i] - beta1[i])*z)*(beta0[i] + beta1[i]) / (2.0*beta1[i]) * aMinus);
